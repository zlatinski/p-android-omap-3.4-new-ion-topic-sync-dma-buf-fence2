...
#ifndef _LINUX_OMAP_GFX_BUF_H_
#define _LINUX_OMAP_GFX_BUF_H_

#define TI_MAX_SUB_ALLOCS 4

#define OMAP_DMM_2D_STRIDE_BYTES_ALIGN	 4096  /* Bytes */
#define OMAP_DMM_1D_STRIDE_BYTES_ALIGN	  128  /* Bytes */
#define OMAP_DSS_STRIDE_PIXELS_ALIGN       8  /* Bytes */
#define OMAP_GPU_HW_STRIDE_PIXELS_ALIGN    8  /* Pixels */
#define OMAP_2D_HW_STRIDE_PIXELS_ALIGN     8  /* Pixels */
#define OMAP_VIDEO_STRIDE_BYTES_ALIGN     32  /* Bytes */
#define OMAP_ISS_STRIDE_BYTES_ALIGN       32  /* Bytes */
...
/* standard rectangle */
typedef struct omap_gfx_buf_rect {
        __s32 x;	/* left */
        __s32 y;	/* top */
        __u32 w;	/* width */
        __u32 h;	/* height */
...
typedef struct omap_gfx_buf_params {
...
} omap_gfx_buf_params_t;

/**
 *
 * omap_gfx_buf_plane_req_t - a single plane buffer allocation request
 *
 * IN parameters:
 * @bpp,
 * @mem_flags,
 ...
 */
typedef struct omap_gfx_buf_plane_req {
        omap_gfx_buf_params_t params;
        __s32 export_fd;
} omap_gfx_buf_plane_req_t;

/**
 * omap_gfx_buf_req_t - buffer allocation request
 *
 * IN parameters:
 * @pixel_format,
 * @planes,
 ...
 * @planes,
 */
typedef struct omap_gfx_buf_req {
        __u32 pixel_format;  /* IN */
        __u64 name;          /* OUT */
        __s32 sync_fd;       /* OUT */
        omap_gfx_buf_plane_req_t planes[TI_MAX_SUB_ALLOCS];
} omap_gfx_buf_req_t;

/**
 * omap_gfx_buf_info_t - buffer info request
 *
 * OUT parameters:
 * @pixel_format
 * @name - global name
 * @planes - planes info
 */
typedef struct omap_gfx_buf_info {
        __u32 pixel_format;
        __u64 name;
        __u32 num_planes;
        omap_gfx_buf_params_t plane_params[TI_MAX_SUB_ALLOCS];
} omap_gfx_buf_info_t;

#define OMAP_GFX_BUF_IOC_MAGIC		'O'

/**
 * DOC: OMAP_GFX_BUF_IOC_PARAMS - get buffer parameters
 *
 * Takes a omap_gfx_buf_info_t structure where the buffer parameters would be returned.
 */
#define OMAP_GFX_BUF_IOC_BUF_PARAMS		_IOR(OMAP_GFX_BUF_IOC_MAGIC, 0, omap_gfx_buf_info_t)

/**
 * DOC: OMAP_GFX_BUF_IOC_SYNC - wait for a gfx_buf to become available
 *
 * pass timeout in milliseconds.  Waits indefinitely timeout < 0.
 */
#define OMAP_GFX_BUF_IOC_SYNC		_IOW(OMAP_GFX_BUF_IOC_MAGIC, 0, __s32)

#ifdef __KERNEL__

/**
 * omap_gfx_buf_plane - syngle plane representation of omap_gfx_buf
 * IN parameters:
 * @params - buffer parameters
 * @dmabuf - dma-buf representing the underlying buffer
 *
 */
typedef struct omap_gfx_buf_plane {
        omap_gfx_buf_params_t params;
        struct dma_buf *dmabuf;
} omap_gfx_buf_plane_t;

/**
 * struct omap_gfx_buf - omap graphics buffer
 ...
 * @planes array of planes, that belong to the omap_gfx_buf.
 */
typedef struct omap_gfx_buf {
        struct file	* file;
        int name;
        ...
        __u32 pixel_format;
        __u32 num_planes;
        omap_gfx_buf_plane_t planes[1];
} omap_gfx_buf_t;

struct omap_gfx_buf_waiter;
typedef void (*omap_gfx_buf_waiter_callback_t)(omap_gfx_buf_t *gfx_buffer,
                struct omap_gfx_buf_waiter *waiter);

/**
 * struct omap_gfx_buf_waiter - metadata for asynchronous waiter on a gfx buffer
 * @waiter_list:	membership in omap_gfx_buf.waiter_list_head
...
struct omap_gfx_buf_waiter {
        ...
        omap_gfx_buf_waiter_callback_t		callback;
};

static inline void omap_gfx_buf_waiter_init(struct omap_gfx_buf_waiter *waiter,
                omap_gfx_buf_waiter_callback_t callback)
{
...
/**
 * omap_gfx_buf_create() - creates a omap graphics buffer
 * @buf_mgr_cxt - context of the buffer manager to allocate from.
 * @req_buf:	requested information for the gfx buffer to create.
 */
int omap_gfx_buf_create(void* buf_mgr_cxt, omap_gfx_buf_req_t* req_buf);


/**
 * omap_gfx_buf_fdget() - get a gfx buffer from an fd
...
/**
 * omap_gfx_buf_put() - puts a refernnce of a omap graphics buffer
 ...
 */
void omap_gfx_buf_put(omap_gfx_buf_t *gfx_buffer);

/**
 * omap_gfx_buf_install() - installs a gfx buffer into a file descriptor
 ...
 */
void omap_gfx_buf_install(omap_gfx_buf_t *gfx_buffer, int fd);

/**
 * omap_gfx_buf_get_fd() - returns a file descriptor for the given gfx_buf
...
/**
 * omap_gfx_buf_wait_async() - registers and async wait on the gfx buffer
 ...
 * @waiter should be initialized with omap_gfx_buf_waiter_init().
 */
int omap_gfx_buf_wait_async(omap_gfx_buf_t *gfx_buffer,
                            struct omap_gfx_buf_waiter *waiter);

/**
 * omap_gfx_buf_cancel_async() - cancels an async wait
 * @gfx_buffer:		gfx buffer to wait on
 * @waiter:		waiter callback
 *
 ...
 */
int omap_gfx_buf_cancel_async(omap_gfx_buf_t *gfx_buffer,
                              struct omap_gfx_buf_waiter *waiter);

/**
 * omap_gfx_buf_wait() - wait on gfx buffer
 * @gfx_buffer:	gfx buffer to wait on
 ...
 * if @timeout < 0
 */
int omap_gfx_buf_wait(omap_gfx_buf_t *gfx_buffer, long timeout);

/**
 * omap_gfx_buf_mgr_shutdown() - closes the buffer mgr on shutdown
 */
void omap_gfx_buf_mgr_shutdown(void);

/**
 * omap_gfx_buf_mgr_init() - Inits the buffer mgr on driver init.
 */
